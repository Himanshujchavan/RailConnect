...
/**
 * The single SeatRepository for the Seat entity. Previously this coexisted with a duplicate
 * com.railconnect.repository.SeatRepository that only WeightedSeatAllocationService used;
 * that duplicate (and its dead-end caller) has been removed and its useful quota-aware
 * queries merged in here.
 */
...
public interface SeatRepository extends JpaRepository<Seat, Long> {
	...
	/**
	 * Find seats in a specific coach that match a given berth type and are not currently
	 * allocated for the provided schedule and journey date.
	 */
	@Query("""
	...
	/**
	 * Find available seats across all coaches that satisfy quota flags (ladies, senior, divyang) and
	 * optional berth. Pass null for any flag you do not want to filter on.
	 * Not wired into the allocation flow yet — kept as groundwork for the quota roadmap item.
	 */
	@Query("""
			SELECT s FROM Seat s
